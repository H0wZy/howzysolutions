...
const enMdPath = join(root, 'public', 'cv.md')
const ptMdPath = join(root, 'public', 'pt', 'cv.md')

describe('Static CV Markdown Generation', () => {
  it('generates public/cv.md and public/pt/cv.md', () => {
  ...
  it('includes valid YAML frontmatter in English', () => {
    ...
    expect(content).toContain('pdf: https://howzysolutions.com/cv.pdf')
  ...
  it('includes valid YAML frontmatter in Portuguese', () => {
    ...
    expect(content).toContain('pdf: https://howzysolutions.com/pt/cv.pdf')
    expect(content).toMatch(/captured_at: \d{4}-\d{2}-\d{2}/)
  })
