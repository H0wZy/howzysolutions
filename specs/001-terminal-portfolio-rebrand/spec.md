...
1. **Given** a visitor on the English site, **When** they switch to Portuguese, **Then** they
   arrive at the Portuguese rendering of the same page, at the same scroll position, and all
   visible prose is in Portuguese. *(Amended 2026-08-23 — see the note below.)*
...
#### Amendment 2026-08-23 — locale is a URL, not a toggle

Scenario 1 originally required the switch to happen **without a page reload**. That was written
before the architecture settled, and it is withdrawn.

Every page is prerendered, and the client ships no framework, so an in-place swap would mean
tagging every text node with a content path and rebuilding the DOM from data on switch —
machinery spread across every component, for a page whose text never changes otherwise.

Giving each locale its own URL (`/` and `/pt/`, `/work/<id>/` and `/pt/work/<id>/`) is better on
everything the requirement was actually protecting:

- A Portuguese page can be **shared** — a link sent to a Brazilian client opens in Portuguese.
- `hreflang` alternates point at **real documents**, which is what FR-018 and T070 need to mean
  anything.
- Each document carries its own `lang` attribute natively, with no JavaScript.
- It costs one navigation of roughly 8 KB, with the script and font already cached.

The part of the scenario that mattered — not losing the visitor's place — is preserved by
storing scroll position across the navigation, so the amended scenario keeps that requirement
and drops only the implementation detail.

FR-016's "without a page reload" phrasing is superseded by this note for the same reason.
...
- **FR-001**: The site MUST present all eight projects: telasparana, selzler-construtora,
  generative-ai-e2, viralvideogen, studiobiasantos, howzysolutions, terminal, and authsys.
